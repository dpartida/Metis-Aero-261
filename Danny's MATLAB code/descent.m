function [B,vxmaxsl,yxmaxd,vxmaxd,xbr,tbr] = descent(Em,hc,hla,W,S,K,Cd0,rho_sl,rho_c)
...
vxmaxsl=sqrt(((2*W/S)/rho_sl)*sqrt(K/Cd0)); %airspeed for best range at sea level

yxmaxd = (1 / Em) * 180 / pi; %slowest descent angle for best range (degrees)

vxmaxd = sqrt(((2 * W / S) / rho_c) * sqrt(K  / Cd0)); %airspeed for best range descent
